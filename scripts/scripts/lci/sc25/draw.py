...
def batch(df):
    ...
    df1_tmp = df[df.apply(lambda row:
                          ...
                          axis=1)]
    df1 = df1_tmp.copy()
    plot(df1, "nthreads", "throughput(mops)", "name2", None,
         dirname=dirname, filename="all", with_error=True,
         x_label="Thread Number", y_label="Throughput (Mop/s)", figsize=(6, 3))
