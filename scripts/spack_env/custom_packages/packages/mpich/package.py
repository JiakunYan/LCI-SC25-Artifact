...
class Mpich(AutotoolsPackage, CudaPackage, ROCmPackage):
    ...
    variant(
        ...
        values=("off", "pmi", "pmi2", "pmix", "cray", "slurm"),
        multi=False,
    )
    variant(
        "device",
...
'-Wl,-flat_namespace'.""",
    ...
    variant(
        "ch4_shmmods", when="device=ch4",
        values=("auto", "none", "posix", "xpmem", "gpudirect"),
        multi=False,
        default="auto"
    )
    ...
    def autoreconf(self, spec, prefix):
        ...
        if self.spec.satisfies("~fortran"):
            args.append("--without-fortran")
        bash = which("bash")
        bash("./autogen.sh", *args)
    ...
    def configure_args(self):
        ...
        if "pmi=off" in spec:
            ...
            config_args.append("--with-pmi2=/opt/cray/pe/pmi/default")
        ...
        if "+debug" in spec:
            config_args.append("--enable-g=debug")
            config_args.append("--enable-fast=O0")

        if not spec.satisfies("ch4_shmmods=auto"):
            config_args.append("--with-ch4-shmmods={0}".format(spec.variants["ch4_shmmods"].value))
